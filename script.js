...
if (butterflyLink) {
  butterflyLink.addEventListener('click', runButterflyTransition);
}

/* Make every product/cart button open the real WhatsApp number */
document.querySelectorAll('.product-info button').forEach((button) => {
  button.addEventListener('click', () => {
    const card = button.closest('.product-card');
    const productName = card?.querySelector('h3')?.textContent?.trim() || 'cake';
    const message = encodeURIComponent(`Hello ZMORD Cake, I want to order: ${productName}`);
    window.open(`https://wa.me/96565005017?text=${message}`, '_blank', 'noopener');
  });
});
